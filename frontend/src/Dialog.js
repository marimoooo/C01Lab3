...
function Dialog({open, initialNote, closeDialog, postNote: postNoteState, patchNote: patchNoteState}) {
    ...
    const patchNote = (_id, updatedTitle, updatedContent) => {
        try {
            fetch(`http://localhost:4000/patchNote/${_id}`, 
            {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json",
              ...
              .then((data) => {
                patchNoteState(data._id, data.title, data.content);
                console.log("Note patched successfully:", data);
              })
              .catch((error) => {
                console.error("Patch request failed:", error);
              });
          } catch (error) {
            console.error("Error in patchNote function:", error);
          }
      };
